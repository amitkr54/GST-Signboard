...
export function MobileTextFormatToolbar({
    ...
    const renderMainMenu = () => (
        <div className={isLandscape ? verticalMenuClasses : "flex flex-col gap-3"}>
            {/* First row: Primary formatting tools */}
            <div className={isLandscape ? "" : "flex items-center justify-around px-2"}>
                {isTextObject && (
                    <>
                        <button onClick={() => {
                            window.dispatchEvent(new CustomEvent('open-mobile-editor'));
